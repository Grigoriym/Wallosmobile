...
class DrawerItemsBuilder {
    ...
    fun build(): ImmutableList<DrawerItem<DrawerDestination>> = persistentListOf(
        DrawerItem.Destination(
            destination = DrawerDestination.Dashboard,
            label = NativeText.Resource(RString.dashboard_title),
            icon = IconSource.Vector(Icons.Filled.Home)
        ),
        DrawerItem.Destination(
            destination = DrawerDestination.Subscriptions,
            label = NativeText.Resource(RString.subscriptions_title),
        ...
        DrawerItem.Destination(
            ...
            label = NativeText.Resource(RString.settings_title),
            icon = IconSource.Vector(Icons.Filled.Settings)
        ),
        DrawerItem.Group(
            label = NativeText.Resource(RString.manage_group_title),
            items = listOf(
                DrawerItem.Destination(
                    destination = DrawerDestination.Categories,
                    label = NativeText.Resource(RString.categories_title),
