...
/**
 * Regression check for 27.1's localization of the Menu content description, on the `Menu` config
 * `SubscriptionsScreen`'s shell uses (`SubscriptionsScreen.kt`'s `NavigationIconConfig.Menu`).
 * `androidDeviceTest` is only wired for `feature:subscriptions:ui` and `core:storage` (27.4) —
 * `uikit` itself has no device-test source set — so this renders `grappim-kit-uikit`'s `TopBar`
 * directly, resolving its content descriptions the same way `AuthenticatedMainScreen`'s shell
 * does, rather than through the full drawer shell, which lives in `composeApp` and can't be
